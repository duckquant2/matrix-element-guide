...
Telegram шифрует только «секретные чаты» один на один, и только на мобильных. Обычные и групповые чаты хранятся на серверах Telegram в открытом виде. На своём VPS с Matrix все комнаты зашифрованы, включая групповые и нет привязки к номеру телефона.

### По сравнению с WhatsApp
WhatsApp использует E2E-шифрование, но сервер принадлежит Meta. Метаданные собираются для рекламного профилирования. Бэкапы в облаке по умолчанию не зашифрованы. Требует номер телефона. На своём сервере все метаданные остаются у тебя.

### По сравнению с Signal
Signal - эталон шифрования, но сервер один и принадлежит фонду Signal Foundation. При блокировке в стране - зависимость от их прокси. Требует номер телефона. На своём VPS ты контролируешь сервер и маршрутизацию.

### Что даёт Matrix на своём VPS
- Полный контроль над данными и инфраструктурой
- Никакой привязки к телефону - полная анонимность
- Обход блокировок через серверы из белых списков
